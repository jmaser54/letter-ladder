...
def write_solutions_js(schedule):
    """Only ever includes days strictly before today's real date, and only
    the tiers that actually have a solution filled in. Today's and future
    puzzles NEVER appear here, so re-running this script can't leak an
    answer to a puzzle that hasn't been played yet."""
    today = date.today()
    past_only = {}
    for day_key, day_entry in schedule.items():
        ...
        if tiers_with_solutions:
            past_only[day_key] = tiers_with_solutions

    with open(SOLUTIONS_OUTPUT_PATH, "w") as f:
        f.write("// AUTO-GENERATED by scripts/generate_schedule.py - do not hand-edit\n")
        f.write("// Only ever contains PAST days (before today's date as of the\n")
        f.write("// last time this script ran) - never today's or future puzzles.\n")
        f.write("const SOLUTIONS_SCHEDULE = ")
        f.write(json.dumps(past_only, indent=2))
        f.write(";\n")
    print(f"Wrote {len(past_only)} past day(s) of solutions to {SOLUTIONS_OUTPUT_PATH}")
